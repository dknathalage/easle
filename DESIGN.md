...
**React path (compile-on-write).** A content node may instead carry `source` — JSX authored as a React component. JSX is compiled to plain JS **once at write time** in the Electron main process via `esbuild` (JSX pragma targets a global `React`, classic runtime, CJS output) and the result is cached in `contents.compiled`; a compile error throws and rolls back the whole `applyOps` batch. Reusable per-document **components** (`createComponent`, referenced by name from a node's `source` via `require('./Name')`) and document-wide shared styles/scripts (`setDocumentAssets` → `{css, js}`) are compiled/stored the same way. A single prebuilt **content-renderer** bundle (React + ReactDOM only — no UI library, no Tailwind, no theme: a blank slate) is loaded inside the iframe; it evals the compiled CJS, resolves shared components by name, injects the shared css/js, and mounts. Compiled code is eval'd only inside the `sandbox="allow-scripts"` iframe, never in the host renderer — **the sandbox model is unchanged**. Legacy `{html, css, js}` content (`source`/`compiled` null) keeps rendering via the raw path, so nothing is rewritten; version snapshots capture `source`/`compiled`, components, and document assets, and `restoreVersion` rebuilds them.
